...
def show(setEffect = None, duration = 30, insequence = False, start = 0):
    ...
    while True:
        if insequence:
            ...
            while effect == oldEffect: effect = rng.integers(1, 38)
        oldEffect = effect
        ...
        # radialGradient
            ...
            colors = [[RED, GREEN, RED], [GREEN, BLUE, GREEN], [BLUE, RED, BLUE]
                      , [YELLOW, PURPLE, YELLOW], [CYAN, PINK, CYAN], [GREEN, PURPLE, GREEN]][rng.integers(0, 6)]
            backwards = rng.choice([True, False])
            if rng.choice([True, False]):
                colors.reverse()
            speed = rng.integers(250, 350)
            func = lambda: gradient(colors = colors, variant = 4)
            fps(func, "radialGradient")
        ...
        # fadeRestore
            ...
            func = lambda: fadeRestore(colors = colors, p = p, halflife = halflife, duration = duration)
            fps(func, "fadeRestore")
        # sweeper
        elif effect == 36:
            variant = rng.integers(2)
            if variant == 0:
                colors = [RED, ORANGE, YELLOW, GREEN, BLUE, PURPLE]
                SEQUENCE = True
            else:
                ...
                SEQUENCE = False
            speed = rng.uniform(4, 10) * rng.choice([-1, 1])

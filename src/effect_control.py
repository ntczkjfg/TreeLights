...
def show(effects = None, duration = 30, in_sequence = False, start = 0):
    old_effect = 0
    effect = start
    cycles = 1
    effect_count = 34
    ...
    while True:
        ...
        # cylinder
            ...
            colors = [COLORS, TRADITIONAL_COLORS, TREE_COLORS,
                      None, TREE_COLORS_2, RAINBOW][rng.integers(6)]
        ...
        # fade
            ...
            colors = [TRADITIONAL_COLORS, COLORS, TREE_COLORS,
                      TREE_COLORS_2, None, rng.choice(COLORS)][rng.integers(6)]
            ...
            func = lambda: fade(colors = colors, midline = midline, amplitude = amplitude,
                                speed = speed, duration = 2*duration)
            fps(func, 'fade')
        # fade_restore
        elif effect == 8:
            colors = [TRADITIONAL_COLORS, COLORS, TREE_COLORS,
        ...
        # pulsating_sphere
            ...
            func = lambda: pulsating_sphere(colors = colors, dr= dr, dh= dh, duration = 3*duration)
            fps(func, 'pulsating_sphere')
        # rain
        elif effect == 12:
            ...
            if variant < 2: # Regular rain
                ...
                speed = 2
                do_fade = True
                name = 'accumulating_snow'
            drop_count = rng.integers(8, 14)
            func = lambda: rain(colors = color, drop_count = drop_count, accumulation_speed = accumulation_speed,
                                wind = wind, speed = speed, do_fade = do_fade, duration = 2*duration)
            fps(func, name)
        # random_planes
        elif effect == 13:
            colors = [None, COLORS, RAINBOW][rng.integers(3)]
            func = lambda: random_planes(colors = colors, duration = 3*duration)
            fps(func, 'random_planes')
        # snake
        elif effect == 14:
            if duration < 10:
                d = duration
        ...
        # spinning_plane
            ...
            if rng.random() < 0.5:
                colors = [None, COLORS, RAINBOW, [WHITE, BLUE],
                          [RED, GREEN], [CYAN, WHITE], [GREEN, BLUE],
                          [BLUE, YELLOW], [YELLOW, PURPLE], [PURPLE, GREEN]][rng.integers(10)]
                ...
                if height == 0 or height == tree.z_max: speed = rng.uniform(1.75, 3)
                two_colors = rng.choice([True, False, False])
                background = rng.choice([True, True, False])
                spinner = False
                name = 'spinning_plane'
            else:
                colors = [[WHITE, BLUE], [RED, GREEN], [CYAN, WHITE],
                          [GREEN, BLUE], [BLUE, YELLOW], [YELLOW, PURPLE],
                          [PURPLE, GREEN]][rng.integers(7)]
        ...
        # spirals
            ...
            if variant == 1:
                ...
                colors = [[RED, ORANGE, YELLOW, GREEN, BLUE, PURPLE],
                          [GREEN, RED], [BLUE, WHITE, CYAN, WHITE],
                          [RED, WHITE, BLUE]][rng.integers(4)]
                spin_count = rng.uniform(1, 3)
            ...
            func = lambda: spirals(colors = colors, variant = variant, spin_count = spin_count,
                                   ...
                                   generate_together = generate_together,
                                   spin_after_done = spin_after_done, duration = 3*duration)
            fps(func, 'spirals')
        # spirals (not spinning)
            ...
            if variant == 1:
                ...
                spin_count = 2
            variant = rng.choice([-1, 1])
            func = lambda: spirals(colors = colors, spin_count = spin_count, variant = variant,
                                   spin_speed = 0, end_after_spirals = True, cycles = cycles, duration = 3*duration)
            fps(func, 'spirals (not spinning)')
            sleep(duration/5)
        # spirals (barbershop pole)
        elif effect == 18:
            if rng.random() < 0.5: # Two spirals
                colors = [[RED, WHITE], [BLUE, GREEN], [RED, GREEN],
                          [BLUE, WHITE], [GREEN, PURPLE], [CYAN, ORANGE],
                          [ORANGE, BLUE], [YELLOW, PURPLE]][rng.integers(8)]
                spin_count = rng.uniform(1, 4)
            else: # More than two spirals
                colors = [[RED, OFF, OFF, OFF, OFF, GREEN, OFF, OFF, OFF, OFF],
            ...
            while time() - start_time < 3*duration:
                if rng.random() < 0.5: # Two spirals
                    ...
                    c = [[WHITE, BLUE], [RED, GREEN], [CYAN, WHITE],
                         ...
                         [PURPLE, GREEN], [RED, BLUE], [GREEN, GREEN],
                         [BLUE, BLUE]][rng.integers(10)]
                    colors = [[c[0], OFF, OFF, OFF, OFF, c[1], OFF, OFF, OFF, OFF],
                              [RED, ORANGE, YELLOW, GREEN, BLUE, PURPLE],
        ...
        # spotlight
            ...
            if rng.random() < 0.15:
                colors = [[RED, GREEN], [GREEN, BLUE], [BLUE, YELLOW],
                          [YELLOW, PURPLE], [PURPLE, GREEN], [RED, BLUE],
                          [GREEN, PURPLE]][rng.integers(7)]
            else:
                colors = [WHITE, BLUE]
        ...
        # sweeper
            ...
            if rng.random() < 0.5:
                ...
                colors = [None, COLORS, TRADITIONAL_COLORS, TREE_COLORS, TREE_COLORS_2][rng.integers(5)]
                sequence = False
        ...
        # twinkle
            ...
            if rng.random() < 0.5:
                colors = [[100, 100, 100], [100, 100, 100], [0, 100, 0],
                          [0, 0, 100], [0, 0, 100], [0, 100, 100],
                          [50, 0, 100]][rng.integers(0, 7)]
            else:
                colors = [COLORS, TREE_COLORS, TREE_COLORS_2, RAINBOW,
                          TRADITIONAL_COLORS, TRADITIONAL_COLORS,
                          rng.choice(COLORS)][rng.integers(7)]
            if rng.random() < 0.5:
        ...
        # wander
            ...
            colors = [None, COLORS, TREE_COLORS, TREE_COLORS_2,
                      RAINBOW, TRADITIONAL_COLORS][rng.integers(6)]
            wander_time = rng.uniform(.5, 2)
            func = lambda: wander(colors = colors, wander_time = wander_time, duration = 3*duration)
            fps(func, 'wander')
        # winding_spirals
        elif effect == 25:
            c = [[WHITE, BLUE], [RED, GREEN], [CYAN, WHITE], [GREEN, BLUE],
                 [BLUE, YELLOW], [YELLOW, PURPLE], [PURPLE, GREEN],
                 [RED, BLUE], [GREEN, GREEN], [BLUE, BLUE]][rng.integers(10)]
            colors = [[RED, ORANGE, YELLOW, GREEN, BLUE, PURPLE],
            ...
            func = lambda: winding_spirals(colors = colors, max_spin_count = max_spin_count,
                                           d_spin = d_spin, d_offset = d_offset, duration = 2*duration)
            fps(func, 'winding_spirals')
        # z_spiral
        elif effect == 26:
            start_time = time()
            ...
            fps(func, 'z_spiral')
            func = lambda: tree.cycle(variant = 4, speed = cycle_speed, backwards = cycle_backwards,
                                      duration = max(2*duration - (time() - start_time), 0.1))
            fps(func, 'cycle')
        # gradient
        elif effect == 27:
            ...
            fps(func, 'gradient')
            func = lambda: tree.cycle(variant = variant, backwards = backwards, speed = speed,
                                      indices = indices, duration = 3*duration)
            fps(func, 'cycle')
        # pizza
        elif effect == 28:
            func = lambda: pizza()
        ...
        # random_fill
            ...
            if variant == 0:
                colors = [None, COLORS, TREE_COLORS, TREE_COLORS_2,
                          rng.choice(COLORS), TRADITIONAL_COLORS][rng.integers(6)]
                ...
                if rng.random() < 0.7:
                    ...
                    colors = [COLORS, TRADITIONAL_COLORS, TREE_COLORS,
                              TREE_COLORS_2, RAINBOW][rng.integers(5)]
            ...
            if duration < 10:
                ...
                d = np.inf
            func = lambda: random_fill(colors = colors, speed = speed, sequence = sequence,
                                       empty = empty, cycles = 2*cycles, duration = d)
            fps(func, name)
        # set_all
        elif effect == 31:
